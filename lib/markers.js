...
exports.get = {
	validate: {
		payload: {
			...
			locationLatMin: Joi.number().required(),
			locationLngMin: Joi.number().required(),
			locationLatMax: Joi.number().required(),
			locationLngMax: Joi.number().required(),
	...
	handler: function(request, reply){
		...
		var latMin = request.payload.locationLatMin;
		var latMax = request.payload.locationLatMax;
		var lngMax = request.payload.locationLngMax;
		var lngMin = request.payload.locationLngMin;
		...
		function doWorking(){
			Markers.find(obj, 'name marker_type lat_lng like_count',function(err, markers){
				if(err) return reply({error: err});
				else{
					// var markersTmp = [];
					// for(var i=0; i<markers.length; i++){
					// 	if((LatLon(lat, lng).distanmarker_type lat_lng like_countceTo(LatLon(markers[i].lat_lng.lat, markers[i].lat_lng.lng), 4)) > dist) continue;
...
exports.getMarkers = {
	...
	handler: function(request, reply){
		var page = request.payload.page?request.payload.page:0;
		var limit = request.payload.limit?request.payload.limit:1;
		var latMin = request.payload.locationLatMin;
		var latMax = request.payload.locationLatMax;
		var lngMax = request.payload.locationLngMax;
		var lngMin = request.payload.locationLngMin;
		...
		function doWorking(){
			Markers.find(obj, 'name marker_type lat_lng like_count').skip(limit*page).limit(limit).exec(function(err, markers){
				if(err) return reply({error: err});
				else{
					reply({
						markers: markers
...
exports.getMarkersWarning = {
	...
	handler: function(request, reply){
		...
		function doWorking(){
			Markers.find(obj, 'name marker_type lat_lng like_count').skip(page*limit).limit(limit).exec(function(err, markers){
				if(err) return reply({error: err});
				else{
					var markersTmp = [];
					for(var i=0; i<markers.length; i++){
